...
function playerDrawCard() {
    let playerCard = getTopCard(deck);
    dealPlayerCard();
    $('.undone')
...
function dealerDrawCard() {
    let dealerCard = getTopCard(deck);
    ;
    $('.undone2')
...
;
function pointLogic(card) {
    let newCard = card[1];
    if (newCard === "A") {
        playerAce += 1;
        return 11;
    }
    else if (newCard === "K" || newCard === "Q" || newCard === "J")
        return 10;
    else
        return parseInt(newCard);
}
;
function hit() {
...
function disableButton() {
    ...
    $('.btn-stand').prop('disabled', true).removeClass('hover');
    if (totalCash >= 200)
        $('.btn-200').prop('disabled', false).addClass('hover');
    if (totalCash >= 100)
        $('.btn-100').prop('disabled', false).addClass('hover');
    if (totalCash >= 50)
        $('.btn-50').prop('disabled', false).addClass('hover');
}
;
